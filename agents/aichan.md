...
| `output_format` | 읽을 PROGRESS 경로 | UI 스펙 파싱 방식 |
|-----------------|----------------------|----------------------|
| `markdown` (default) | `.shinchan-docs/{DOC_ID}/PROGRESS.md` | `## Phase N:` H2 + 코드블록 |
...
본 에이전트의 UI 구현 결과 보고(component 테스트/breakpoint/접근성)는 양 모드 동일 — 단, html 경로의 경우 himawari가 받은 보고를 `<tr class="ts-change-log">` 행으로 변환하여 PROGRESS.html에 기록한다.
